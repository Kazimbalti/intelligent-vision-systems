...
def find_ball(frame):
    ...
    hsv = None  # TODO 1: BGR -> HSV
    mask = None  # TODO 2: colour mask from --lower / --upper
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        return None, mask
    c = None  # TODO 3: the largest contour
    if cv2.contourArea(c) < args.min_area:
        return None, mask
    M = cv2.moments(c)
    cx, cy = None, None  # TODO 4: centroid
...
while True:
    ...
    if found:
        ...
        dx, dy = None, None  # TODO 5: offset of the ball from the image centre
    ...
    for i in range(1, len(trail)):                         # fading trail
        if trail[i - 1] is None or trail[i] is None:
            continue
        pass  # TODO 6: connect consecutive trail points
